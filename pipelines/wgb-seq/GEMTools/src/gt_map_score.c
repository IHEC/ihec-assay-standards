...
GT_INLINE void gt_map_set_phred_score(gt_map *const map,
                                      const uint8_t phred_score) {
  GT_MAP_CHECK(map);
  map->phred_score = phred_score;
}

uint64_t gt_map_calculate_gt_score(gt_alignment *al, gt_map *map,
                                   gt_map_score_attributes *ms_attr) {
  ...
  GT_MAP_ITERATE(map, map_block) {
    GT_MISMS_ITERATE(map_block, misms) {
      ...
      if (has_qualities) {
        quality_misms =
            gt_string_get_string(quals)[misms->position] - qual_offset;
        if (quality_misms > GT_MAP_SCORE_MAX_QUALITY)
          quality_misms = GT_MAP_SCORE_MAX_QUALITY;
        else if (quality_misms < map_cutoff ||
                 gt_string_get_string(read)[misms->position] == 'N')
          quality_misms = 0;
      } else
        quality_misms = GT_MAP_SCORE_MISSING_QUALITY;
  ...
  if (score > GT_MAP_SCORE_MAX_GT_SCORE)
    score = GT_MAP_SCORE_MAX_GT_SCORE;
  return score;
}
...
void gt_map_score_delete_map_score_tmap_buf(
    gt_map_score_tmap_buf *ms_tmap_buf) {
...
void gt_map_score_heap_sink_down(gt_map_score_tmap **base, uint64_t sz) {
  ...
  while (1) {
    ...
    if (k >= sz)
      break;
    if (k + 1 < sz && base[k - 1]->al_score < base[k]->al_score)
      k++;
...
void gt_map_pair_template(gt_template *template,
                          gt_map_score_attributes *ms_attr,
                          gt_map_score_tmap_buf *ms_tmap_buf) {
  ...
  for (rd = 0; rd < 2; rd++) {
    ...
    if (al[rd]) {
      ...
      if (ms_attr->max_strata_searched) {
        ...
        if (max_complete_strata > limit)
          max_complete_strata = limit;
      }
  ...
  if (nmap[0] + nmap[1]) {
    ...
    gt_cond_fatal_error_msg(
        sz > ms_tmap_buf->size,
        "Fatal error - gt_map_score_tmap_buf set too small\n");
    uint64_t idx = 0;
    uint64_t i = 0;
    char *map_flag[2];
    ...
    int indx1;
    for (indx1 = 0; indx1 < nmap[0]; indx1++) {
      mmap[0] = *gt_vector_get_elm(al[0]->maps, indx1, gt_map *);
      int indx2;
      for (indx2 = 0; indx2 < nmap[1]; indx2++) {
        mmap[1] = *gt_vector_get_elm(al[1]->maps, indx2, gt_map *);
        gt_status gt_err;
        int64_t x = gt_template_get_insert_size(mmap, &gt_err, 0, 0);
        if (gt_err == GT_TEMPLATE_INSERT_SIZE_OK &&
            x >= ms_attr->minimum_insert && x <= ms_attr->maximum_insert) {
          uint64_t gt_score = mmap[0]->gt_score | (mmap[1]->gt_score << 16);
          uint32_t pair_score = mmap[0]->gt_score + mmap[1]->gt_score;
          if (ms_attr->insert_phred) {
            pair_score += ms_attr->insert_phred[x - ms_attr->minimum_insert];
            gt_score |=
                ((uint64_t)ms_attr->insert_phred[x - ms_attr->minimum_insert]
                 << 32);
          }
          if (idx < sz) {
            tmap_buf[idx].gt_score = gt_score;
            tmap_buf[idx].al_score = pair_score;
            tmap_buf[idx].map_idx[0] = indx1;
            tmap_buf[idx].map_idx[1] = indx2;
            //          Insert pointer into tmap heap so we can recover the
            //          worst easily
            tmaps[idx] = tmap_buf + idx;
            gt_map_score_heap_float_up(tmaps, ++idx);
          } else {
            // We've reached the limit of paired maps, so we only add this one
            // if it is better than the worst found so far
            if (tmaps[0]->al_score > pair_score) {
              tmap_buf[0].gt_score = gt_score;
              tmap_buf[0].al_score = pair_score;
              tmap_buf[0].map_idx[0] = indx1;
              tmap_buf[0].map_idx[1] = indx2;
    ...
    nmap[2] = idx;
    if (idx) {
      // Sort tmaps in decreasing score order using the fact that they should be
      // in a valid heap
      i = idx;
      while (i > 1) {
      ...
      for (i = 0; i < idx; i++) {
        ...
        attr.distance =
            gt_map_get_global_distance(map1) + gt_map_get_global_distance(map2);
    ...
    // Now we pick up orphan single end maps, sort on score, and use no more
    // than the first
...
/*
 * To check for duplicate alignments we make a key out of a concatenation of
 *sequence name, position and strand.  If we have
 * multiple segments, keys for all segments are concatenated.  Keys for paired
 *alignments are formed from the two keys
...
static size_t gt_map_score_get_key_size(gt_map *map) {
  ...
  GT_MAP_SEGMENT_ITERATOR(map, map_segment_iterator) {
    nbl++;
    ksize += gt_string_get_length(
        gt_map_segment_iterator_get_map(&map_segment_iterator)->seq_name);
  }
...
static size_t gt_map_score_make_pair_key(void *buf, gt_map *map1,
                                         gt_map *map2) {
...
void gt_map_calculate_template_mapq_score(gt_template *template,
                                          gt_map_score_attributes *ms_attr) {
  typedef struct {
    char *key;
    gt_map *map;
  ...
  for (rd = 0; rd < 2; rd++) {
    ...
    if (ms_attr->max_strata_searched) {
      uint32_t limit = ms_attr->max_strata_searched + 1;
      if (max_complete_strata[rd] > limit)
        max_complete_strata[rd] = limit;
    }
  }
  size_t buf_len = 1024;
  char *buf = gt_malloc(buf_len);
  bool unpaired = false;
  bool unpd[2] = {false, false};
  uint64_t min_score[6] = {0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff};
  {
    GT_TEMPLATE_ITERATE_MMAP__ATTR_(template, maps, maps_attr) {
      if (!maps_attr)
        gt_fatal_error(TEMPLATE_NOT_SCORED);
      uint64_t score = maps_attr->gt_score;
      if (score == GT_MAP_NO_GT_SCORE)
        gt_fatal_error(TEMPLATE_NOT_SCORED);
      uint64_t seq_like[2], interval_like;
      seq_like[0] = score & 0xffff;
      seq_like[1] = (score >> 16) & 0xffff;
      interval_like = (score >> 32) & 0xff;
      // Build up list of single end alignments (need this so we can scale the
      // MAPQ score)
      // Use hash to avoid counting a single end alignment twice if it occurs in
      // two paired alignments
      for (rd = 0; rd < 2; rd++) {
        if (maps[rd]) {
          ...
          if (!mp_hash) {
            ...
            if (seq_like[rd] < min_score[rd])
              min_score[rd] = seq_like[rd];
            if (!maps[rd ^ 1]) {
              if (seq_like[rd] < min_score[rd + 4])
                min_score[rd + 4] = seq_like[rd];
              unpd[rd] = true;
            }
          }
        }
      }
      if (maps[0] && maps[1]) { // True paired alignments.  Shouldn't need to
                                // check for duplicates, but we will anyway
  ...
  // Get score of best possible aligning read that we didn't look for with the
  // mapping parameters
  uint64_t fake_sc[3] = {0, 0, 0};
  for (rd = 0; rd < 2; rd++)
    if (max_complete_strata[rd]) {
      gt_alignment *al = gt_template_get_block(template, rd);
      if (!al)
        continue;
      ...
      size_t k = 0;
      int quals_offset =
          ms_attr->quality_format == GT_QUALS_OFFSET_33 ? 33 : 64;
      ...
      if (k >= max_complete_strata[rd]) {
        ...
        for (i = 0; i < max_complete_strata[rd]; i++)
          fake_sc[rd] += qvs[i];
      }
      free(qvs);
    }
  if (unpaired) {
    // For paired reads, the best aligning read that we didn't find would be the
    // best *unpaired* read found from one end combined
    // with the fake score from the other with the most likely interval size
    // (i.e. 0)
    if (unpd[0] && unpd[1]) {
      uint64_t f1 = fake_sc[0] + min_score[5];
      uint64_t f2 = fake_sc[1] + min_score[4];
      fake_sc[2] = f1 < f2 ? f1 : f2;
    } else if (unpd[0]) {
      fake_sc[2] = fake_sc[1] + min_score[4];
    } else {
      fake_sc[2] = fake_sc[0] + min_score[5];
    }
    uint64_t f1 = fake_sc[0] + fake_sc[1];
    if (f1 < fake_sc[2])
      fake_sc[2] = f1;
  }
  ...
  for (rd = 0; rd < 3; rd++)
    if (fake_sc[rd]) {
      ...
      mp_hash->map = 0;
      HASH_ADD_KEYPTR(hh, mhash[rd], mp_hash->key, key_size, mp_hash);
      if (fake_sc[rd] < min_score[rd])
        min_score[rd] = fake_sc[rd];
    }
  ...
  for (rd = 0; rd < 3; rd++)
    if (mhash[rd]) {
      ...
      for (mp_hash = mhash[rd]; mp_hash; mp_hash = mp_hash->hh.next) {
        mp_hash->prob = exp(PHRED_KONST *
                            ((double)(mp_hash->score) - (double)min_score[rd]));
        z += mp_hash->prob;
      }
      for (mp_hash = mhash[rd]; mp_hash; mp_hash = mp_hash->hh.next) {
        mp_hash->prob /= z;
        if (1.0 - mp_hash->prob < 1.0e-255)
          mp_hash->phred = 254;
        else {
  ...
  // To see how well supported our paired reads are compared to all unpaired
  // combinations,
  // if unpaired alignments exist we look at all combinations of single end
  // reads
  if (unpaired) {
    map_hash *mp1, *mp2;
    ...
    GT_TEMPLATE_ITERATE_MMAP__ATTR_(template, maps, maps_attr) {
      for (rd = 0; rd < 2; rd++)
        if (maps[rd]) {
          ...
          HASH_FIND(hh, mhash[rd], buf, key_size, mp_hash);
          assert(mp_hash);
          GT_MAP_ITERATE(maps[rd], mapb) {
            mapb->phred_score = mapb->se_phred_score = mp_hash->phred;
          }
        }
      if (maps[0] && maps[1]) {
        ...
        HASH_FIND(hh, mhash[2], buf, key_size, mp_hash);
        assert(mp_hash);
        // We say that a paired end MAPQ can not be more than the sum of the
        // single end scores
        uint64_t zsc = maps[0]->phred_score + maps[1]->phred_score;
        if (mp_hash->phred > zsc)
          mp_hash->phred = zsc;
        if (mp_hash->phred1 > zsc)
          mp_hash->phred1 = zsc;
        maps_attr->phred_score = mp_hash->phred;
        maps_attr->pair_score = unpaired ? mp_hash->phred1 : mp_hash->phred;
        // Copy paired MAPQ value to single end maps
        maps[0]->phred_score = maps[1]->phred_score = mp_hash->phred;
      }
    }
  }
  free(buf);
...
void gt_map_calculate_alignment_mapq_score(gt_alignment *alignment,
                                           gt_map_score_attributes *ms_attr) {
  uint32_t map_cutoff = ms_attr->mapping_cutoff;
  ...
  // Build up list of single end alignments (need this so we can scale the MAPQ
  // score)
  ...
  if (nmaps) {
    ...
    GT_ALIGNMENT_ITERATE(alignment, map) {
      ...
      if (score == GT_MAP_NO_GT_SCORE)
        gt_fatal_error(ALIGNMENT_NOT_SCORED);
      maplist[k].score = score & 0xffff;
      maplist[k].map = map;
      if (maplist[k].score < min_score)
        min_score = maplist[k].score;
      k++;
    }
    // Get score of best possible aligning read that we didn't look for with the
    // mapping parameters
    ...
    if (max_complete_strata) {
      ...
      if (k >= max_complete_strata) {
        qsort(qvs, k, sizeof(int), gt_map_cmp_int);
        for (i = 0; i < max_complete_strata; i++)
          fake_sc += qvs[i];
      }
      free(qvs);
      maplist[nmaps].score = fake_sc;
      maplist[nmaps].map = NULL;
      if (fake_sc < min_score)
        min_score = fake_sc;
    }
    double z = 0.0;
    for (k = 0; k <= nmaps; k++) {
      maplist[k].prob =
          exp(PHRED_KONST * ((double)(maplist[k].score) - (double)min_score));
      z += maplist[k].prob;
    }
    for (k = 0; k < nmaps; k++) {
      maplist[k].prob /= z;
      int tp;
      if (1.0 - maplist[k].prob < 1.0e-255)
        tp = 254;
      else {
      ...
      GT_MAP_ITERATE(maplist[k].map, mapb) {
        mapb->phred_score = mapb->se_phred_score = tp;
      }
    }
    free(maplist);
  }
